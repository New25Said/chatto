...
app.post("/reset", (req, res) => {
  ...
  console.log("ÃƒÂ¢Ã…Â¡ ÃƒÂ¯Ã‚Â¸Ã‚Â Chat reseteado manualmente");
  res.sendStatus(200);
});

io.on("connection", (socket) => {
  console.log("ÃƒÂ¢Ã…â€œÃ¢â‚¬Â Usuario conectado:", socket.id);
...
socket.on("chat private", (msg) => {
  ...
  if (targetId) {
    ...
    chatHistory.push(message);
    saveHistory();
    socket.emit("chat message", message); // tÃƒÆ’Ã‚Âº ves tu mensaje
...
socket.on("chat group", (msg) => {
  ...
  socket.on("disconnect", () => {
    console.log("ÃƒÂ¢Ã…â€œÃ¢â‚¬â€œ Usuario desconectado:", socket.id);
...
server.listen(PORT, () => console.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â Servidor chat listo en puerto ${PORT}`));
